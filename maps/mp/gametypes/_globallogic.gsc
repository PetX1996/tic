...
prematchPeriod()
{
	...
	{
		// matchStartTimer();//fuck this shit
		matchStartTimerSkip();
	}
	else
	{
...
Callback_StartGameType()
{
	...
	thread maps\mp\gametypes\_main::init();
...
//	self thread fakeLag();
	...
	if ( level.oldschool )
	{
		self setClientDvars( "ragdoll_explode_force", 0,
...
Callback_PlayerDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	if(isDefined(level.preparing))
	{
		if(isDefined(eAttacker))
		{
			...
			{
				self.health = self.maxhealth;
				eattacker finishPlayerDamage(self, self, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir , sHitLoc, psOffsetTime);
			}
		}
	}
...
//	self maps\mp\gametypes\_weapons::updateWeaponUsageStats();
	...
	body thread staythere(self.origin);
	...
	if ( isDefined( self.switching_teams ) )
	{
		maps\mp\gametypes\_main::releaseClass( self.pers["team"], self.class );
		if ( isDefined( self.curClass ) && self.curClass != self.class )
			maps\mp\gametypes\_main::releaseClass( self.pers["team"], self.curClass );
...
delayStartRagdoll( ent, sHitLoc, vDir, sWeapon, eInflictor, sMeansOfDeath )
{
	...
	while(isDefined(ent) && !isDefined(level.roundstarted))
	{
		ent delete();
		wait 0.05;
	}
