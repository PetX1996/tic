...
init()
{
	...
	game["popup_identify"] = "popup_identify";
	game["traitor"] = "traitor";
	game["missingaction"] = "missingaction";
	game["other"] = "other";

	precacheMenu("other");
	precacheMenu("popup_identify");
	precacheMenu("missingaction");
	precacheMenu("traitor");
...
onMenuResponse()
{
	...
	for(;;)
	{
		...
		{
			self maps\mp\gametypes\_main::processLoadoutResponse( response );
			continue;
		}
			...
			case "spectator":
				...
				if(!isDefined(level.allowSpec))
				{
					...
					if(isDefined(level.roundStarted))
					{
						...
						{
							level.players[i] iprintln("^1" + self.name + " joined spectator");
						}
					}
				}
				break;
		...
		{
			if ( !self maps\mp\gametypes\_main::verifyClassChoice( self.pers["team"], response ) )
				continue;

			self maps\mp\gametypes\_main::setClassChoice( response );
			self closeMenu();
			self closeInGameMenu();
		...
		else if( menu == game["menu_changeclass"] )
		{
			...
			self maps\mp\gametypes\_main::menuAcceptClass();
		}
		else if ( !level.console )
		{
			...
			else if(menu == game["traitor"])
				maps\mp\gametypes\_quickmessages::traitor(response);
			else if(menu == game["popup_identify"])
				maps\mp\gametypes\_quickmessages::identify(response);
			else if(menu == game["other"])
				maps\mp\gametypes\_quickmessages::other(response);
			else if(menu == game["missingaction"])
				maps\mp\gametypes\_quickmessages::missing(response);
		}
		
		switch(response)
		{						
			case"1":maps\mp\gametypes\_quickmessages::wm_quickmessage(response);continue;
		}
	}
}